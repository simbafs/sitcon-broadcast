...
func (sq *SessionQuery) QueryNext() *SessionQuery {
	query := (&SessionClient{config: sq.config}).Query()
...
func (sq *SessionQuery) WithNext(opts ...func(*SessionQuery)) *SessionQuery {
	query := (&SessionClient{config: sq.config}).Query()
...
func (sq *SessionQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Session, error) {
	...
	if query := sq.withNext; query != nil {
		if err := sq.loadNext(ctx, query, nodes,
			func(n *Session) { n.Edges.Next = []*Session{} },
			func(n *Session, e *Session) { n.Edges.Next = append(n.Edges.Next, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (sq *SessionQuery) loadNext(ctx context.Context, query *SessionQuery, nodes []*Session, init func(*Session), assign func(*Session, *Session)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(session.NextTable)
		s.Join(joinT).On(s.C(session.FieldID), joinT.C(session.NextPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(session.NextPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(session.NextPrimaryKey[0]))
	...
	for _, n := range neighbors {
		nodes, ok := nids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected "next" node returned %v`, n.ID)
		}
		for kn := range nodes {
			assign(kn, n)
		}
